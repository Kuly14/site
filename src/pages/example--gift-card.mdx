...
- [x] Using [Blaze](https://github.com/butaneprotocol/blaze-cardano/) with [Blockfrost](https://blockfrost.io)<sup>★</sup>.
  <sub>
...
use cardano/transaction.{Transaction, PolicyId}
...
use cardano/transaction.{OutputReference, Transaction, PolicyId}
...
use cardano/transaction.{OutputReference, PolicyId, Transaction}
...
Let's import our new `readValidators` file into `src/routes/+page.server.ts` file and use it to
in a server side loader. This will allow us to access the data in the `+page.svelte` page component
...
Blaze also requires initialization so let's get some boilerplate out of the way.
    ...
    {:else}
      <form class="mt-10 grid grid-cols-1 gap-y-8" onsubmit={submitTokenName}>
        ...
        {#if tokenName.length > 0}
          <Button type="submit">Make Contract</Button>
        {/if}
      </form>
    {/if}
  </div>
